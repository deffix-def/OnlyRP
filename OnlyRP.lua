...
local checkbox_show_phone = new.bool(inisettings.settings.show_phone)
local checkbox_show_phone_sms = new.bool(inisettings.settings.show_phone_sms)
...
function sampev.onServerMessage(clr, text)
    ...
    if inisettings.settings.state then
        ...
        if clr == -192134401 and text:find('%w+_%w+ поднял трубку мобильного') then
            return true
        end
        ...
        elseif clr == -65281 and text:find('SMS') and text:find('Получатель: %w+_%w+ Тел.: %d+') then
            if not inisettings.settings.show_phone_sms then
                return false
            end
        elseif (clr == -65281 and text:find('Телефон | %w+_%w+:')) or (clr == -1 and text:find('%w+_%w+%[%d+%] %[Телефон%] говорит: ')) then
            if not inisettings.settings.show_phone then
                return false
            end
        elseif clr == -577699926 and text:find('%w+_%w+ ') then
...
imgui.OnFrame(function()
    ...
    imgui.SetNextWindowSize(imgui.ImVec2(300, 417), imgui.Cond.Always)
    if canMove == 0 then
    ...
    imgui.PushFont(big)
    imgui.SetCursorPosX((300 - imgui.CalcTextSize('OnlyRP').x) / 2)
    imgui.Text('OnlyRP')
    imgui.PopFont()
    ...
    if imgui.CustomCheckbox(u8'Показывать телефонный разговор', checkbox_show_phone) then
        inisettings.settings.show_phone = checkbox_show_phone[0]
        inicfg.save(inisettings, inicfgfile)
    end
    if imgui.CustomCheckbox(u8'Показывать смс по номеру', checkbox_show_phone_sms) then
        inisettings.settings.show_phone_sms = checkbox_show_phone_sms[0]
        inicfg.save(inisettings, inicfgfile)
    end
